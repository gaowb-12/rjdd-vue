<template>
    <div class="unit_exercise">
        ...
        <div class="subject" v-if="choice">
            <div v-for="item in showData" :key="item.question.text">
                ...
                <div class="option">
                    ...
                    <div class="option_div" :class="{borderB: borderBNo}" @click="flag && clickOption($event, 'C', item.answer)">
        ...
        <!-- 主观题 -->
        <div class="subject" v-if="subjective">
            <div v-for="item in showData" :key="item.question.text">
                <!-- 题干 -->
                <div class="matter">
                    ...
                    <flexbox orient="vertical">
                        <!-- 文字 -->
                        <flexbox-item v-if="item.question.text" style="padding-top: 20px;"><pre>{{item.question.text}}</pre></flexbox-item>
                        <!-- 图片 -->
                        <flexbox-item v-else>
                            <img :src="item.question.img" alt="">
                        </flexbox-item>
                ...
                <div class="answer" v-if="showAnswer">
                    ...
                    <flexbox class="correct" orient="vertical">
                        <!-- 文字 -->
                        <flexbox-item v-if="item.answer.text"><pre>{{item.answer.text}}</pre></flexbox-item>
                        <!-- 图片 -->
                        <flexbox-item v-else>
                            <img :src="item.answer.img" alt="">
                        </flexbox-item>
                    </flexbox>

                    <!-- 方法总结 -->
                    <flexbox class="resolution">
                        <flexbox-item><i class="iconfont icon-chakanjiexi"></i> 方法总结</flexbox-item>
                    </flexbox>

                    <flexbox class="resolution_panel" orient="vertical">
                        <!-- 文字 -->
                        <flexbox-item v-if="item.summary.text"><pre>{{item.summary.text}}</pre></flexbox-item>
                        <!-- 图片 -->
                        <flexbox-item v-else>
                            <img :src="item.summary.img" alt="">    
                        </flexbox-item>
                    </flexbox>
                </div>
            </div>
...
import { GetMethodSkill } from '@/api/http';
...
export default {
    ...
    created () {       
        // 请求数据
        // GetMethodSkill({},'1221001502141010000_2')   // mock测试
        GetMethodSkill({},this.$route.params.category_id)
        .then(res => {
            this.list = res.techniques;
